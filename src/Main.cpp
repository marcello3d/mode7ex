...
short actionsInfos[]=
		...
		IDMN_ACTION_WRAP_ON,			M_ACTION_WRAP_ON,			ACT_ACTION_WRAP_ON, 			0, 0,
		IDMN_ACTION_WRAP_OFF,			M_ACTION_WRAP_OFF,			ACT_ACTION_WRAP_OFF, 			0, 0,
...
short expressionsInfos[]=
		...
		IDMN_EXP_GET_WRAP,				M_EXP_GET_WRAP,				EXP_GET_WRAP, 				0, 0,
		...
		IDMN_EXP_GET_MAPHEIGHT,			M_EXP_GET_MAPHEIGHT,		EXP_GET_MAPHEIGHT, 			0, 3, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, M_ACT_SX, M_ACT_SY, M_ACT_SHEIGHT,
		...
		IDMN_EXP_GET_MAPWIDTH2,			M_EXP_GET_MAPWIDTH2,		EXP_GET_MAPWIDTH2, 			0, 4, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, M_ACT_SX, M_ACT_SY, M_ACT_SWIDTH, M_ACT_SHEIGHT,
		IDMN_EXP_GET_MAPHEIGHT2,		M_EXP_GET_MAPHEIGHT2,		EXP_GET_MAPHEIGHT2, 		0, 4, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, M_ACT_SX, M_ACT_SY, M_ACT_SWIDTH, M_ACT_SHEIGHT,
		IDMN_EXP_GET_MAPVISIBLE2,		M_EXP_GET_MAPVISIBLE2,		EXP_GET_MAPVISIBLE2, 		0, 4, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, EXPPARAM_LONG, M_ACT_SX, M_ACT_SY, M_ACT_SWIDTH, M_ACT_SHEIGHT,
		};
...
int getPixel2(LPSURFACE surface, int width, int height, bool repeat, float x, float y) {
	int xi=(int)x;
	int yi=(int)y;
	float xf=x-xi;
	float yf=y-yi;
	...
	int xp=0, yp=0, c1=0, c2=0, c3=0, c4=0, c12, c34;
	int xi1=repeat?MOD1(xi+1,width):min(xi+1,width);
	if (xf!=0) {
		xp=(int)(xf*0xFF);
		surface->GetPixel(xi, yi,c1);
	...
	int yi1=repeat?MOD1(yi+1,height):min(yi+1,height);
	if (yf!=0) {
		yp=(int)(yf*0xFF);
...
BYTE getPixel256(LPSURFACE surface, int width, int height, bool repeat, float x, float y) {
	int xi=(int)x;
	int yi=(int)y;
	float xf=x-xi;
	float yf=y-yi;
...
#define SETPIX(x,y,c)	(rdPtr->rResMode?setPixel(rdPtr,x,y,c):rdPtr->rSf->SetPixel(x,y,c))
void setPixel(LPRDATA rdPtr, int x, int y, COLORREF color) {
	...
	if (rdPtr->rResMode==0) { rdPtr->rSf->SetPixel(x,y,color); return; }
	int xx,yy;
	if (rdPtr->rResMode==1) {
...
void RedrawScene(LPRDATA rdPtr, bool redraw) {
	...
	//	COLORREF	fadeColor = RGB(0,0,0);
		...
		if (rdPtr->rElevation)
		for (int y=rdPtr->rElevation>=0; y<=vlimit; ) {
		//for (int y=rdPtr->rHo.hoImgHeight; --y>0;) {}
			double v=(double)abs(rdPtr->rElevation)/(rdPtr->rElevation<0?rdPtr->rHo.hoImgHeight-y:y);
			...
			for (int x=0; x<rdPtr->rHo.hoImgWidth; x+=rdPtr->rResX) 
				...
				if (rdPtr->rMipmap) {
					int yy = y+1;
					double cv=(double)abs(rdPtr->rElevation)/(rdPtr->rElevation<0?rdPtr->rHo.hoImgHeight-yy:yy);
		...
		MessageBox(0,buf,"mode 7 debug",MB_OK);
				...
				if (rdPtr->rRepeat && interpolate) {
					imgx=MOD2(imgx,rdPtr->rImageWidth);
					imgy=MOD2(imgy,rdPtr->rImageHeight);
				}
				int xi = (int)imgx;
				int yi = (int)imgy;
				if (rdPtr->rRepeat && !interpolate) {
					xi=MOD1(xi,rdPtr->rImageWidth);
					yi=MOD1(yi,rdPtr->rImageHeight);
				}
					...
					if (rdPtr->rMipmap) {
						...
						if (distance<=1) {
							if (rdPtr->rInterpolate)
								tempColor = getPixel2(rdPtr->rImage,rdPtr->rImageWidth,rdPtr->rImageHeight,rdPtr->rRepeat,imgx,imgy);
							else
							...
							int w=1<<i,h=1<<i;
							if (rdPtr->rInterpolate)
								tempColor = getPixel2(rdPtr->rMipmaps[i],w,h,rdPtr->rRepeat,imgx*w/rdPtr->rImageWidth,imgy*h/rdPtr->rImageHeight);
							else
				...
				if (tempColor>0) {
					...
					if (rdPtr->rThickness) {
						...
						if (y+rdPtr->rResY>=vlimit) {
							...
							if (rdPtr->rRepeat && rdPtr->rInterpolateHeight) {
								cimgx=MOD2(cimgx,rdPtr->rImageWidth);
								cimgy=MOD2(cimgy,rdPtr->rImageHeight);
							}
							int cxi = (int)cimgx;
							int cyi = (int)cimgy;
							if (rdPtr->rRepeat && !rdPtr->rInterpolateHeight) {
								cxi=MOD1(cxi,rdPtr->rImageWidth);
								cyi=MOD1(cyi,rdPtr->rImageHeight);
							}

							COLORREF tempColor3 = 0;
							if (rdPtr->rRepeat || (cxi>=0 && cxi<rdPtr->rImageWidth && cyi>=0 && cyi<rdPtr->rImageHeight))
							...
							if (tempColor3) {
								if (rdPtr->rImageHNum>=-1) {
									...
									//getPixel(rdPtr->rImageH,);
									thick2 = (thick2*rvalue)>>8;
								}
								//tempColor = RGB(255,0,0);
							} else /*if (rdPtr->rThicknessMode) */thick2=0;
						...
						y2+=rdPtr->rResY;
						for (int i=y1; i<=y2; i+=rdPtr->rResY)
							setPixel(rdPtr,x,i,tempColor);
					} else
					...
					if (rdPtr->rInterpolate) {
						...
						if (!rdPtr->rMipmap2||distance<=2)
							tempColor = getPixel2(rdPtr->rImage2,rdPtr->rImage2Width,rdPtr->rImage2Height,1,xi2,yi2);
						else {
							...
							i=max(i,0);
							int w=1<<i,h=1<<i;
							tempColor = getPixel2(rdPtr->rMipmaps2[i],w,h,rdPtr->rRepeat,xi2*w/rdPtr->rImage2Width,yi2*h/rdPtr->rImage2Height);
						}
					} else {
						int xii=(int)xi2;
						int yii=(int)yi2;
...
short WINAPI DLLExport Act_WrapOn(LPRDATA rdPtr, long param1, long param2)
{
	if (rdPtr->rRepeat) return 0;
	rdPtr->rRepeat = true;
	if (rdPtr->rAutoRedraw) rdPtr->rNeedRedraw=true;
	return 0;
}
short WINAPI DLLExport Act_WrapOff(LPRDATA rdPtr, long param1, long param2)
{
	if (!rdPtr->rRepeat) return 0;
	rdPtr->rRepeat = false;
	if (rdPtr->rAutoRedraw) rdPtr->rNeedRedraw=true;
...
long WINAPI DLLExport Exp_GetWrap(LPRDATA rdPtr,long param1) {
	return rdPtr->rRepeat;
}
...
long WINAPI DLLExport Exp_GetMapXZ(LPRDATA rdPtr,long param1)
{
	...
	double y1 = p3/yy;
	double x1 = ((-rdPtr->rHo.hoImgWidth*xx)/(p3/y1))+xoff;
...
long WINAPI DLLExport Exp_GetHeightAt(LPRDATA rdPtr,long param1) {
	...
	int xi=x,yi=y;
	if (rdPtr->rRepeat) {
		xi=MOD1(x,rdPtr->rImageWidth);
		yi=MOD1(y,rdPtr->rImageHeight);
	}
	if (rdPtr->rRepeat || (xi>=0 && xi<rdPtr->rImageWidth && yi>=0 && yi<rdPtr->rImageHeight)) {
		rdPtr->rImage->GetPixel(xi, yi, tempColor);
		if (tempColor) {
...
long WINAPI DLLExport Exp_GetMapWidth2(LPRDATA rdPtr,long param1)
{
	long p1 = CNC_GetFirstExpressionParameter(rdPtr, param1, TYPE_INT);
	long p2 = CNC_GetNextExpressionParameter(rdPtr, param1, TYPE_INT);
	long w = CNC_GetNextExpressionParameter(rdPtr, param1, TYPE_INT);
...
long WINAPI DLLExport Exp_GetMapHeight2(LPRDATA rdPtr,long param1)
{
	long p1 = CNC_GetFirstExpressionParameter(rdPtr, param1, TYPE_INT);
	long p2 = CNC_GetNextExpressionParameter(rdPtr, param1, TYPE_INT);
	long w = CNC_GetNextExpressionParameter(rdPtr, param1, TYPE_INT);
...
long WINAPI DLLExport Exp_GetMapVisible2(LPRDATA rdPtr,long param1)
{
	long p1 = CNC_GetFirstExpressionParameter(rdPtr, param1, TYPE_INT);
	long p2 = CNC_GetNextExpressionParameter(rdPtr, param1, TYPE_INT);
	long w = CNC_GetNextExpressionParameter(rdPtr, param1, TYPE_INT);
